...
import { getWBSById, getPhasesByWBS } from "../../data/mock-progress-data";

/**
 * Phase view route component.
 *
 * Displays phase-level breakdown for a specific WBS item:
 * - Breadcrumb navigation (Dashboard > WBS)
 ...
 */
export const Route = createFileRoute("/wbs/$wbsId")({
...
function PhaseViewPage() {
  const { wbsId } = useParams({ from: "/wbs/$wbsId" });
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-muted-foreground">WBS Not Found</h2>
          <p className="text-muted-foreground mt-2">The requested WBS item could not be found.</p>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-semibold">{wbsItem.percentComplete}%</span>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="text-2xl font-bold font-mono">{wbsItem.earnedMH.toFixed(1)} MH</div>
          </div>

          <div className="space-y-2">
            <div className="text-sm font-medium text-muted-foreground">Man-Hours Total</div>
            <div className="text-2xl font-bold font-mono">{wbsItem.totalMH.toFixed(1)} MH</div>
          </div>
        </div>
      </div>
        ...
        {phases.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {phases.map((phase) => (
              <Link
                key={phase.id}
                to="/wbs/$wbsId/phase/$phaseId"
                params={{ wbsId, phaseId: phase.id }}
